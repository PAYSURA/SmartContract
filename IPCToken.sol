...
contract PurchasableToken is StandardToken, Pausable {
    ...
    // minimum amount of ether you have to spend to buy some token
    uint256 public minimumEtherAmount = 0.01 ether;
    address public vendorWallet = msg.sender;
    uint256 public exchangeRate = 5000; // 5000 tokens = 1 ether
    uint256 public tokenDecimals;
    
    /** Constructor considers the decimals of the token */
    function PurchasableToken(uint256 _tokenDecimals) public {
        tokenDecimals = _tokenDecimals;
        exchangeRate = safeMul(exchangeRate, 10**tokenDecimals);
    }
    
    /** @dev modifier to allow token purchase only when purchase is unlocked */
    ...
    function buyIPC() payable isPurchasable whenNotPaused public returns (uint256) {
        ...
        // amount of token should not have more than 'tokenDecimals' decimals
...
contract IPCToken is UpgradeableToken, PurchasableToken, Withdrawable {
    ...
    function IPCToken (
        ...
        ) public UpgradeableToken(msg.sender) PurchasableToken(decimals) {
